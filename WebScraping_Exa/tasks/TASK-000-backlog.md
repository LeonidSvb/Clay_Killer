...
# Сессия 2026-03-31 (продолжение) — hotfixes после ручного теста

## Протестировано вручную
- LLM enrichment на реальных данных — работает end-to-end, прогресс-бар в реальном времени
- Save to table — новые колонки появились с жёлтым фоном

## Фиксы по результатам теста
- **Промпты разделены по папкам:**
  - `prompts/enrichment/` → только Streamlit-панель (8 промптов скопированы из CLI)
  - `prompts/` → только CLI, больше не показывается в dropdown панели
  - Детект стиля промпта теперь по контенту: `{text}` в шаблоне = legacy mode
- **Fill remaining — Clay-like UX:**
  - После сохранения сгенерированных колонок — над таблицей появляются кнопки
    `[icp_score — fill 490 empty]` по одной на каждую колонку с пустыми строками
  - Клик → панель открывается с пред-выбором "Fill missing" и нужной колонкой
  - Новый Row mode "Fill missing" в панели запускает enrichment только на пустых строках
...
### 9. Prompt separation (проверить отдельно)
- [ ] В dropdown панели нет `system_context` и нет дублей
- [ ] Все 8 промптов видны: company_full, company_deep, company_profile, icp_score, icp_filter, industry, pain_point, current_method
- [ ] [+ New] создаёт промпт в `prompts/enrichment/`, не в `prompts/`
- [ ] После [Delete] промпт удаляется из `prompts/enrichment/`, файл в `prompts/` остаётся нетронутым

### 10. Fill remaining (проверить после Save to table)
- [ ] После Save: над таблицей появляется кнопка `[icp_score — fill N empty]`
- [ ] Кнопок столько, сколько сгенерированных колонок с пустыми строками
- [ ] Клик → панель открывается, Row mode = "Fill missing", колонка уже выбрана
- [ ] Caption показывает реальное количество пустых строк
- [ ] [Run] обрабатывает только пустые строки (уже заполненные не трогает)
- [ ] Если все строки заполнены — кнопка для этой колонки не показывается
